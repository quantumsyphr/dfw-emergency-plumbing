...
export default function HomePage() {
  return (
    <>
      ...
      <section className="relative bg-slate-50 dark:bg-gradient-to-b dark:from-slate-900 dark:to-slate-950 py-16 px-4 overflow-hidden">
        <div className="absolute top-0 right-1/3 h-48 w-64 rounded-full bg-blue-100/50 dark:bg-cyan-500/10 blur-3xl" />
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
            {DFW_CITIES.map((city) => (
              <Link
                key={city.slug}
                href={`/${city.slug}`}
                className="flex items-center gap-2 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 dark:backdrop-blur-sm px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-200 transition-all hover:border-blue-300 dark:hover:border-blue-500/30 hover:bg-blue-50 dark:hover:bg-white/10 shadow-sm dark:shadow-none"
              >
