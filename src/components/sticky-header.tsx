  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? "bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 shadow-lg shadow-black/5 dark:shadow-black/10"
    ...
    >
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            <div className="relative">
              ...
              {isCityDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 rounded-xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200 dark:border-white/10 shadow-xl py-1">
                  {DFW_CITIES.map((city) => (
                    <Link
                      key={city.slug}
                      href={`/${city.slug}`}
                      className="block px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors"
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-white/10 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl">
          <div className="px-4 py-4 space-y-3">
            <a href={COMPANY.phoneHref} className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-white">
              <Phone className="size-4" />
              {COMPANY.phone}
            </a>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Service Areas</p>
              <div className="grid grid-cols-2 gap-1">
                {DFW_CITIES.map((city) => (
                  <Link
                    key={city.slug}
                    href={`/${city.slug}`}
                    className="px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 rounded-md transition-colors"
